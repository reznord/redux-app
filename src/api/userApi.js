...
export function getUserFollowing(userName) {
  return (dispatch, getState) => {
    ...
    const processResponse = (response) => {
      ...
      dispatch(userActions.receivedUserFollowing(params, response, data));
    };

    const isUserFollowingCached = makeUserCacheChecker('following');
    const following = !isUserFollowingCached(state, params);

    if (following) {
      dispatch(userActions.fetchUserFollowing());
      request
        .get(`${BASE_USERS}/${userName}/following`)
        .then(response => processResponse(response))
        .catch(error => console.error(error, error.stack));
    } else if (!following) {
      const cachedSummary = storeSelector.getCachedUserFollowing(state, params);
      processResponse(cachedSummary);
    }
  };
}
